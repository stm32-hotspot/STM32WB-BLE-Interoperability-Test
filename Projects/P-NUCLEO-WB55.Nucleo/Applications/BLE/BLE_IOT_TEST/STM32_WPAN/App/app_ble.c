...
uint8_t TimerSendNotification_Id;
uint8_t TimerSendIndication_Id;
...
uint8_t a_AdvData[13] =
{
  ...
  9, AD_TYPE_COMPLETE_LOCAL_NAME, 'I', 'O', 'T', '_', 't', 'e', 's', 't',  /* Complete name */
...
void APP_BLE_Init(void)
{
  ...
  APP_DBG_MSG("  \r\n\r");APP_DBG_MSG("** INTEROPERABILITY PROJECT **  \n");
  APP_DBG_MSG("  \r\n");APP_DBG_MSG("** START ADVERTISING **  \n");
  APP_DBG_MSG("  \r\n");APP_DBG_MSG("** NAME : IOT SERVER **  \r\n\r");
...
void BLE_SVC_L2CAP_Conn_Update(uint16_t ConnectionHandle)
{
  /* USER CODE BEGIN BLE_SVC_L2CAP_Conn_Update_1 */
    ...
    float a_ConnInterval[9] = {7.5, 20, 45, 200, 400, 600, 1000, 2000, 4000};
    float Sup_TO_Table[9] = {0x258, 0x258, 0x258, 0x258, 0x258, 0x258, 0x258, 0x3C8, 0xC80};
    float Supervision_Timeout;
          
    if(mutex == 1) {
        mutex = 0;
        uint16_t interval_min = CONN_P(a_ConnInterval[counter]);
        uint16_t interval_max = CONN_P(a_ConnInterval[counter]);
        uint16_t slave_latency = L2CAP_SLAVE_LATENCY;
        uint16_t timeout_multiplier = (uint16_t)Sup_TO_Table[counter]; //L2CAP_TIMEOUT_MULTIPLIER;
        tBleStatus result;
        ...
        {
          APP_DBG_MSG(" CONNECTION UPDATE REQUEST \n");
          APP_DBG_MSG("INTERVAL = %.2f ms \n",a_ConnInterval[counter]);
          APP_DBG_MSG("LATENCY = 0x%x \n",slave_latency);
          Supervision_Timeout = Sup_TO_Table[counter] * 10; //timeout_multiplier * 10;
...
void BLE_SVC_L2CAP_Conn_Update_Slave_Latency(void)
{
  float Conn_Interval[6] = {48.75,48.75,200,200,1000};
  uint8_t Slave_Latency_Table[6] = {2,4,2,4,1};
  float Sup_TO_Table[9] = {0x258, 0x258, 0x258, 0x258, 0x3C8};
  float Supervision_Timeout;
...
/* USER CODE END BLE_SVC_L2CAP_Conn_Update_1 */
  if(mutex == 1) {
    mutex = 0;
    uint16_t interval_min = CONN_P(Conn_Interval[counterc]);
    uint16_t interval_max = CONN_P(Conn_Interval[counterc]);
    uint16_t slave_latency = Slave_Latency_Table[counters];
    uint16_t timeout_multiplier = (uint16_t)Sup_TO_Table[counterc];
    tBleStatus result;
    ...
    {
      APP_DBG_MSG(" CONNECTION UPDATE REQUEST \n");
      APP_DBG_MSG("INTERVAL = %.2f ms \n",Conn_Interval[counterc]);
      APP_DBG_MSG("LATENCY = 0x%x \n",slave_latency);
      Supervision_Timeout = Sup_TO_Table[counterc] * 10; //timeout_multiplier * 10;
      APP_DBG_MSG("SUPERVISION_TIMEOUT = %.2f ms \r\n\r",Supervision_Timeout);
      if (counterc < 4)
      {
        counterc++;
...
void BLE_SVC_Security_Req(void)
{
    tBleStatus result;
    ...
    {
#if(CFG_DEBUG_APP_TRACE != 0)
      APP_DBG_MSG("aci_gap_slave_security_req(), Successfully \r\n\r");
#endif
    }
    else
    {
#if(CFG_DEBUG_APP_TRACE != 0)
      APP_DBG_MSG("aci_gap_slave_security_req(), Failed \r\n\r");
#endif
    }
    return;
